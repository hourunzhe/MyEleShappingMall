        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.trades.length; i++) {
                ...
                if (data.trades[i].state == 2) {
                    ...
                    action = "<span id = 'delete' onclick ='deleteCart(\"" + data.trades[i].id + "\")'style='color: red' class ='glyphicon glyphicon-trash'></span>&nbsp;&nbsp;" +
                ...
                if (data.trades[i].state == 3) {
                    ...
                    action = "<span id = 'delete' onclick ='deleteCart(\"" + data.trades[i].id + "\")'style='color: red' class ='glyphicon glyphicon-trash'></span>&nbsp;&nbsp;" +
                        "<span id = 'pay' onclick ='pay(\"" + data.trades[i].id + "\")'style='color: #337AB7' class ='glyphicon glyphicon-yen'></span>&nbsp;&nbsp;" +
                        "<span id = 'pay' onclick ='discuss(\"" + data.trades[i].id + "\")'style='color: orange' class ='glyphicon glyphicon-comment'></span>"
                }
                $("#tbody").append(
                    ...
                    "<td id = 'action_" + data.trades[i].id + "' style='height: 100px;line-height: 100px'>" + action + "</td>" +
                    "</tr>"
                );
            }
            loadPage(pageCount, data.page)
        }
    });
}
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data == "success") {
                ...
                $("#action_" + tradeId).html("<span id = 'delete' onclick ='deleteCart(\"" + tradeId + "\")'style='color: red' class ='glyphicon glyphicon-trash'></span>&nbsp;&nbsp;" +
                    "<span id = 'pay' onclick ='pay(\"" + tradeId + "\")'style='color: #337AB7' class ='glyphicon glyphicon-yen'></span>&nbsp;&nbsp;" +
                    "<span id = 'pay' onclick ='discuss(\"" + tradeId + "\")'style='color: orange' class ='glyphicon glyphicon-comment'></span>");
                return;
            }
            if (data == "hadPay") {
                alert("您已经支付了该订单!");
